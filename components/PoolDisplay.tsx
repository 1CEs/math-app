...
export default function PoolDisplay() {
  ...
  return (
    <View className="p-4 bg-background-bright flex-1 gap-y-2 rounded-xl">
      <Divider content="Rewards Pool" />
      <ScrollView
        horizontal={true}
        showsHorizontalScrollIndicator={false}
        className="h-28 bg-background-dark p-4 rounded-xl"
      >
        <View className="flex flex-row gap-x-4 items-center">
          {Object.keys(RarityColors).map((rarity, index) => (
            <View 
              ...
              key={index} 
              className="flex-1 items-center justify-center w-20 h-20 bg-background-dark border-4 border-background-bright rounded-xl">
